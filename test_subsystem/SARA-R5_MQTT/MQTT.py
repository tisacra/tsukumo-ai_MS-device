...
def setup():
    ...
    send_at_command(ser, "AT+UPSDA=1,3", timeout=60)

    
    #send_at_command(ser, "AT+UMQTT?", timeout=60)

    #send_at_command(ser, "AT+USOSEC=?", timeout=60)
    ...
    send_at_command(ser, "AT+UMQTTC=1", wait_for="+UUMQTTC:", timeout=60)
    #time.sleep(10)
...
def send_message(ser, topic, message):
    send_at_command(ser, 'AT+UMQTTC=2,0,0,0,"'+topic+'","'+message+'"')

def main():
    # 4. パブリッシュ（メッセージ送信）の例
    # トピック "sensor/temperature" に "25.5°C" を送信（QoS 0, retain 0）
    send_message(ser, MQTT_TOPIC, "25.5°C")
    send_at_command(ser, "AT+UMQTTER", timeout=60)
